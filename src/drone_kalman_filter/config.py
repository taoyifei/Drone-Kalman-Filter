...
class PluginConfig:
    ...
    def __post_init__(self) -> None:
        ...
        """
        self._validate_smoother_mode()
        self._validate_main_window()
        self._validate_kalman_parameters()
        self._validate_prefilter_parameters()
        self._validate_window_relations()

    def _validate_smoother_mode(self) -> None:
        """校验平滑模式是否在支持列表内。
        ...
        Raises:
            ValueError: 当平滑模式不在支持列表中时抛出。
        """
        if self.smoother_mode not in {"kalman", "robust_prefilter_kalman"}:
            raise ValueError(
                "smoother_mode must be 'kalman' or "
                "'robust_prefilter_kalman'"
            )

    def _validate_main_window(self) -> None:
        """校验主链路窗口与时间参数。
    ...
    def _validate_kalman_parameters(self) -> None:
        """校验 Kalman 模型相关参数。
        ...
        if self.hard_residual_speed_mps <= self.soft_residual_speed_mps:
            raise ValueError(
                "hard_residual_speed_mps must be larger than "
                "soft_residual_speed_mps"
            )
        _validate_minimum(
            "soft_noise_scale",
    ...
    def _validate_prefilter_parameters(self) -> None:
        """校验预处理层参数。
        ...
        if self.prefilter_hard_speed_mps <= self.prefilter_soft_speed_mps:
            raise ValueError(
                "prefilter_hard_speed_mps must be larger than "
                "prefilter_soft_speed_mps"
            )
        _validate_positive(
            "prefilter_hard_distance_m",
        ...
        if (
            self.prefilter_bridge_center_distance_m
            <= self.prefilter_bridge_neighbor_distance_m
        ):
            raise ValueError(
        ...
        if (
            self.prefilter_median_window_size < 1
            or self.prefilter_median_window_size % 2 == 0
        ):
            raise ValueError(
                "prefilter_median_window_size must be a positive odd integer"
            )

    def _validate_window_relations(self) -> None:
        """校验窗口与滞后之间的关系。
