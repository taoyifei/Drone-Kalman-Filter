...
class ParsedMessage:
    ...
    def is_smoothable(self) -> bool:
        return self.track_key is not None and self.event_time is not None and self.has_valid_coordinates
...
def parse_message(arrival_seq: int, message: dict[str, Any]) -> ParsedMessage:
    ...
    track_key = TrackKey(target_id, device_id) if target_id and device_id else None
    ...
    return ParsedMessage(
        ...
        longitude_text=longitude_text if isinstance(longitude_text, str) else None,
        message=cloned,
    )
...
def set_position_strings(parsed: ParsedMessage, latitude: float, longitude: float) -> dict[str, Any]:
...
def format_like(reference: str | None, value: float, default_decimals: int = 6) -> str:
